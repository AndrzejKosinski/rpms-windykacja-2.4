/**
 * NIEZALEŻNY SKRYPT INICJUJĄCY DLA NOWEGO KONTA GOOGLE - v12.0.0 (DDS + CMS + Settings)
 * Plik: src/integration/skrypty_GAS/initialization_v6.gs
 * Przygotowuje foldery i pliki z uwzględnieniem DDS.
 */

function setupNewGoogleAccount_v6() {
  console.log("=== Rozpoczęcie inicjalizacji konta (v12.0.0) ===");
  ...
  // 5. Inicjalizacja CMS i DDS Database
  const cmsDbName = 'RPMS_CMS_DATABASE';
  let cmsSS;
  const cmsFiles = rootFolder.getFilesByName(cmsDbName);
  if (cmsFiles.hasNext()) {
    cmsSS = SpreadsheetApp.openById(cmsFiles.next().getId());
    console.log("Znaleziono istniejącą bazę CMS: " + cmsSS.getId());
  } else {
    cmsSS = SpreadsheetApp.create(cmsDbName);
    DriveApp.getFileById(cmsSS.getId()).moveTo(rootFolder);
    console.log("Utworzono nową bazę CMS: " + cmsSS.getId());
  }

  let cmsSheet = cmsSS.getSheetByName('CMS_Content');
  if (!cmsSheet) {
    cmsSheet = cmsSS.getSheets()[0];
    cmsSheet.setName('CMS_Content');
    cmsSheet.appendRow(["Klucz", "Wartość", "Typ"]);
    cmsSheet.getRange("A1:C1").setFontWeight("bold").setBackground("#d9ead3");
    console.log("Utworzono arkusz CMS_Content");
  }

  let ddsSheet = cmsSS.getSheetByName('DDS_Config');
  if (!ddsSheet) {
    ddsSheet = cmsSS.insertSheet('DDS_Config');
    ddsSheet.appendRow(["Klucz", "Wartość", "Typ", "Opis"]);
    ddsSheet.getRange("A1:D1").setFontWeight("bold").setBackground("#d9ead3");
    console.log("Utworzono arkusz DDS_Config");
  }

  // 6. Zapis ID jako Script Properties
  ...
  console.log("=== INICJALIZACJA v12.0.0 ZAKOŃCZONA SUKCESEM ===");
}
