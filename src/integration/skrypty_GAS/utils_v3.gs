/**
 * Plik: src/integration/skrypty_GAS/utils_v3.gs
 * Wersja: 10.7.0 (Hierarchical DMS)
 * Rozszerzony o obsługę hierarchicznej struktury folderów dłużników.
 */
...
/**
 * Tworzy hierarchiczną strukturę folderów dla dłużnika
 * @param {string} parentFolderId - ID folderu głównego klienta (KLIENCI/[Nazwa])
 * @param {string} debtorName - Nazwa dłużnika
 * @param {string} caseId - ID sprawy (dla unikalności)
 * @returns {Object} Obiekt z ID utworzonych folderów
 */
function createDebtorFolderStructure(parentFolderId, debtorName, caseId) {
  try {
    const parentFolder = DriveApp.getFolderById(parentFolderId);
    const folderName = debtorName + " (" + caseId + ")";
    const debtorRoot = getOrCreateFolder(parentFolder, folderName);
    
    const invoicesFolder = getOrCreateFolder(debtorRoot, "01_Faktury");
    const legalFolder = getOrCreateFolder(debtorRoot, "02_Dokumenty_Prawne");
    const correspondenceFolder = getOrCreateFolder(debtorRoot, "03_Korespondencja");
    
    return {
      root: debtorRoot.getId(),
      invoices: invoicesFolder.getId(),
      legal: legalFolder.getId(),
      correspondence: correspondenceFolder.getId()
    };
  } catch (e) {
    console.error("Błąd tworzenia struktury folderów dłużnika: " + e.toString());
    return null;
  }
}
