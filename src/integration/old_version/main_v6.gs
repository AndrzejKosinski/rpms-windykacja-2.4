/**
 * RPMS MULTI-TENANT ENGINE v11.0.0 (Hierarchical DMS + Settings)
 * Plik: src/integration/skrypty_GAS/main_v6.gs
 * Zawiera obsługę hierarchicznej struktury folderów dłużników oraz nowe akcje ustawień firmy.
 * Zaktualizowano obsługę GET_CMS o parametr view.
 */
...
function doPost(e) {
  try {
    ...
    switch(action) {
      ...
      // AKCJE USTAWIEN (SETTINGS)
      case 'GET_COMPANY_DATA':        return getCompanyData(payload);
      case 'UPDATE_COMPANY_DATA':     return updateCompanyData(payload);

      // AKCJE SPRAW I DLUZNIKOW
      ...
      case 'UPLOAD_FILE':        return handleUploadFile(payload);
...
function doGet(e) {
  try {
    ...
    if (params.action === 'GET_CMS' || !params.email) {
      ...
      return handleGetCMSContent(params.view);
    }
