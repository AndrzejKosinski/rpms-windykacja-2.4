...
function updateCompanyData(payload) {
  try {
    ...
    // Synchronizacja: Zapis NIP również w kolumnie B (2) - NIP rejestrowy/Nazwa
    if (data.nip) {
      const nipRange = userSheet.getRange(rowIndex, 2);
      nipRange.setNumberFormat("@"); // Wymuszenie formatu tekstowego
      nipRange.setValue(data.nip.toString());
    }
