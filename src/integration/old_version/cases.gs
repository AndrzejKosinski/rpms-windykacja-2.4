/**
 * Plik: src/cases.gs
 * Moduł zarządzania wierzytelnościami z logiką konsolidacji spraw.
 */
  ...
  try {
    ...
    payload.items.forEach(item => {
      ...
      if (!caseId) {
        ...
        caseSheet.appendRow([caseId, debtorId, "Nowa sprawa", "Standardowa", new Date(), payload.userEmail.toLowerCase()]);
    ...
    return createJsonResponse({ status: 'error', message: "Błąd zapisu w cases.gs: " + err.toString() }); 
  }
}
...
function handleFetchDashboard(email) {
  try {
    ...
    const cases = [];
    for (let i = 1; i < casesData.length; i++) {
      const [caseId, debtorId, status, strategy] = casesData[i];
      const debtor = debtorsData.find(d => d[0] === debtorId);
      if (debtor) {
        const cInv = groups[caseId] || [];
        cases.push({
          caseId, debtorName: debtor[2], nip: debtor[1], address: debtor[3],
          status, strategy, 
