...
import { ArrowRight, ShieldCheck, Zap, Gavel, Play, Star } from 'lucide-react';
...
const Hero: React.FC<HeroProps> = ({ id, onRegister, onOpenLawyer, data }) => {
  const title = data?.title || "Odzyskaj Swoje Pieniądze Zanim Znikną.";
  ...
  return (
    <section id={id || 'hero'} className="relative min-h-screen flex items-center pt-32 pb-20 overflow-hidden bg-white">
      ...
      <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-brand-blue/5 rounded-full blur-[120px] translate-x-1/3 -translate-y-1/3"></div>
      <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-brand-light-blue/20 rounded-full blur-[100px] -translate-x-1/3 translate-y-1/3"></div>
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 w-full relative z-10">
        <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
          ...
          <div className="lg:w-3/5 space-y-10 text-center lg:text-left">
            <div className="inline-flex items-center gap-2 px-5 py-2 bg-brand-navy/5 text-brand-navy text-[11px] font-black rounded-full uppercase tracking-[0.3em] animate-in fade-in slide-in-from-left duration-700">
              <Zap size={14} className="text-brand-blue fill-brand-blue" /> Hybrydowy Model Windykacji B2B
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 pt-4 animate-in fade-in slide-in-from-left duration-700 delay-300">
              <button 
                onClick={() => onRegister('hero_primary')}
                className="group relative px-10 py-6 bg-brand-navy text-white rounded-[var(--radius-brand-button)] font-black text-lg shadow-2xl shadow-brand-navy/30 hover:bg-brand-blue transition-all overflow-hidden w-full sm:w-auto"
              >
                <div className="relative z-10 flex items-center justify-center gap-3">
                  Rozpocznij windykację <ArrowRight size={20} className="group-hover:translate-x-2 transition-transform" />
                </div>
          ...
          {/* Prawa kolumna: Animacja / Wizualizacja */}
          <div className="lg:w-2/5 relative animate-in zoom-in duration-1000 delay-200">
            <HeroAnimation />
            
            {/* Pływające karty statusu */}
            <div className="absolute -top-10 -right-10 p-6 bg-white/80 backdrop-blur-xl border border-white rounded-[var(--radius-brand-card)] shadow-2xl animate-float">
            ...
            <div className="absolute -bottom-10 -left-10 p-6 bg-brand-navy text-white rounded-[var(--radius-brand-card)] shadow-2xl animate-float-delayed">
               <div className="flex items-center gap-4">
