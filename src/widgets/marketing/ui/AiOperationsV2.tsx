...
interface AiOperationsV2Props {
  id?: string;
}

const AiOperationsV2: React.FC<AiOperationsV2Props> = ({ id }) => {
  const [imgError, setImgError] = useState(false);

  return (
    <section id={id || 'ai_operations_v2'} className="min-h-screen py-20 lg:py-24 bg-slate-50 relative overflow-hidden flex flex-col justify-center">
      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
        <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
          ...
          <div className="lg:w-1/2 relative group">
            ...
            <div className="absolute -bottom-8 -right-4 lg:-right-8 p-6 bg-white/90 backdrop-blur-xl border border-white rounded-[var(--radius-brand-card)] shadow-2xl z-20 animate-in slide-in-from-right duration-700 delay-300">
              <div className="flex items-center gap-4 mb-3">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Status Egzekucji</p>
                  <p className="text-brand-navy font-black text-base">98% Skuteczności EPU</p>
                </div>
              </div>
              <div className="flex gap-1.5">
                {[1, 2, 3, 4, 5].map(i => (
                  <div key={i} className={`h-1 flex-1 rounded-full ${i <= 4 ? 'bg-brand-blue' : 'bg-slate-200'}`}></div>
                ))}
              </div>
            </div>
          ...
          <div className="lg:w-1/2 space-y-10">
            <h2 className="text-4xl lg:text-7xl text-brand-navy leading-[1.1] tracking-tight">
            ...
            <p className="text-base lg:text-xl text-slate-500 font-medium leading-relaxed max-w-xl">
